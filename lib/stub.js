 ...
 *   //    if 'returnValue' is an error or inherits from error, it will throw it, rather than return it.
...
module.exports = ( function () {
  ...
  function isErrorType ( obj ) {
    if ( toType( obj ) == "error" )
      ...
      return isErrorType( obj.prototype );
    else
      return false;
  }
  ...
  function Stub ( responseArgs, synchronous ) {
    ...
    var mock = function () {
      ...
      if ( !synchronous ) {
        ...
        // Check if the first argument is of type error
        if ( responseArgs[0] && isErrorType( responseArgs[0] ) ) {
          throw responseArgs[0];
        } else {
          return responseArgs[0] || responseArgs[1];
        }
      }
    };
